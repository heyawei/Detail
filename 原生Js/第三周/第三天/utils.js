var utils = (function () {
    ...
    return {
        ...
        //setGroupCss给元素设置一组样式
        setGroupCss: function (curEle, opt) {
            for (var attr in opt) {
                this.setCss(curEle, attr, opt[attr]);
            }
        },
        //css:1)获取curEle，attr，2）设置一个curEle，attr,value,3)设置一组：curEle,opt
        css: function (curEle) {
            var argTwo = arguments[1];
            if (typeof argTwo === 'string') {
                var argThr = arguments[2];
                if (typeof argThr === 'undefined') {
        ...
        //获取该前元素下的所有子元素，筛选出某个标签的元素集合
       getChild:function(curEle,ele){
           var ary=[];
           var child=curEle.childNodes;
           for(var i=0;i<child.length;i++){
               var cur=child[i];
              if(cur.nodeType===1){
                  if(typeof ele!=='undefined'){
                      if(cur.toUpperCase()===ele.nodeName){
                         ary.push(cur)
                      }
                  }
              }
