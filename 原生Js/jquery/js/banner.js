...
$(function(){
    ...
    function bind(){
        ...
        $.each(data,function(index,item){
            strImg+='<img3 src="" realImg="'+item.imgSrc+'" alt="">';
            strLi+=index==0?'<li class="on"></li>':'<li></li>';
        });
        strImg+='<img3 src="" realImg="'+data[0].imgSrc+'" alt="">';
        $boxInner.html(strImg).css('width',(data.length+1)*1000);
        $ul.html(strLi);
    }
    //3.延迟加载
    $aImg=$boxInner.find('img3');
    ...
    function bannerTip(){
        ...
        $.each($aLi,function(index,item){
            //原生Js==tmp?$(item).addClass('on'):$(item).removeClass('on');//JQ
