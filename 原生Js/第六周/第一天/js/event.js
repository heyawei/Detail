...
function on(ele,type,fn){//用来绑定事件；
    if(/^self/.test(type)){//都是跟自定义事件有关的方法；--自定义事件绑定
        ...
        if(ele.addEventListener){//标准浏览器
            ...
            if(!ele['aEvent'+type]){
                ele['aEvent'+type]=[];
                ele.attachEvent('on'+type,function(){//解决重复绑定的问题，并且执行一次
                    run.call(ele)//改变this问题
                });//给系统事件池只存了一个run方法；
            }
...
function  run(){//run方法只在IE下执行。目的：把自己事件池中的所有方法都顺序调用；--被系统事件触发的；
    var e=window.event;
    var type= e.type;
...
//功能：改变函数中的this指向；并且返回函数的定义阶段。
function processThis(fn,context){
    return function (e){
        fn.call(context,e)//fn.call相当于move.call(content,e)
    }
}
