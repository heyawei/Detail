...
EventEmitter.prototype.off=function(type,fn){//解除自定义绑定
    var a=this['aEmitter'+type];//拿到数组
    if(a && a.length){
        for(var i=0; i< a.length; i++){//循环解除事件
            if(a[i]==fn){
                a[i]=null;
                break;
            }
...
Drag.prototype=new EventEmitter;//这样写的缺点：Drag.prototype被污染；导致constructor不正确
