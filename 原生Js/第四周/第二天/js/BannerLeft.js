...
function Banner(id,opt){//私有变量：把普通函数中的全局变量，都作为构造函数的私有变量
    ...
    this.aImg=this.oBoxInner.getElementsByTagName('img3');
    ...
    this.timer=null;
    this.n=0;
    this.url=this.opt.url||'json/data1.txt';
...
Banner.prototype={
    ...
    bind:function  bind(){
        ...
        for(var i=0; i<this.data.length; i++){
            strImg+='<img3 src="" realImg="'+this.data[i].imgSrc+'" alt="">';
            strLi+=i==0?'<li class="on"></li>':'<li></li>';
        }//记得在拼接完图片之后，还需要再多拼接一张图片，同时改变oBoxInner的宽度
        strImg+='<img3 src="" realImg="'+this.data[0].imgSrc+'" alt="">';
